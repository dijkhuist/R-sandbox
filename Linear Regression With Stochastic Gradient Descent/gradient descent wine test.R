...
ptm <- proc.time()
gdec.eta1 = gradientR(y = y, X = data.frame(x1,x2,x3, x4,x5), eta = 100, iters = 10)

filename='winequality-white.csv'
dataset<-read.csv(file=filename,sep=',',header=T)
dataset <- scale(dataset)

y=dataset[12]
y=unlist(y)
gdec.etal2=gradientR(y=y, X =data.frame(dataset$alcohol,dataset$fixed_ac,dataset$volatile_ac,dataset$citric_ac,
                                        dataset$residu_sugar,dataset$chlor,dataset$free_sulfur,dataset$total_sulfur,
                                        dataset$dens,dataset$ph), eta=100,iters=10)
